...
- **Permission updated/asked** — auto-approved (configurable, supports both API versions)
...
If OpenCode rejects the configured override (for example `Model not found`), Spacebot retries that prompt once using OpenCode's default model selection for the session.
